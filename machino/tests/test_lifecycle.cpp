...
void test_failed_start_rolls_back() {
    ...
    // no automatic retry loop happened in between
    LCHECK(r.log.count("platform.bring_up") == 1);
...
// FAILED is terminal for the life of the process. Every case below uses
// fail_times = 1, so the FIRST bring-up fails and any later one would SUCCEED:
// if the manager retried at all, these tests would see a running pipeline.
// That is the point - the OOM of 2026-09-22 came from retries that each cost
// ~1.2 MB, and a reconnecting consumer is an unbounded source of them.
void test_failed_is_sticky() {
    // 1. demand is held while the pipeline fails, and time keeps passing
    {
        Rig r;
        auto keep = r.mgr.acquire(ConsumerType::Rtsp);          // base up, demand 1
        LCHECK(keep.active() && r.log.count("platform.bring_up") == 1);
        r.platform.fail_at = FakePlatform::FailAt::BringUp; r.platform.fail_times = 1;
        std::string err;
        Result up = r.mgr.update_stream(r.stream, true, err);   // controlled restart that fails
        LCHECK(!up && r.mgr.state() == State::Failed);
        LCHECK(r.mgr.stats().total_demand == 1);                // the consumer is still holding
        const int after_fail = r.log.count("platform.bring_up");
        // the grace timer is the only thing that ticks: it must not restart us
        r.mgr.on_grace_timeout();
        r.mgr.on_unit_grace(UNIT_MAIN);
        LCHECK(r.mgr.state() == State::Failed);
        LCHECK(r.log.count("platform.bring_up") == after_fail); // NO further attempt
    }

    // 2. a second consumer arrives while the state is FAILED
    {
        Rig r;
        r.platform.fail_at = FakePlatform::FailAt::BringUp; r.platform.fail_times = 1;
        Result res; auto d = r.mgr.acquire(ConsumerType::Rtsp, &res);
        LCHECK(!d.active() && !res && r.mgr.state() == State::Failed);
        LCHECK(r.log.count("platform.bring_up") == 1);
        Result res2; auto second = r.mgr.acquire(ConsumerType::HttpStream, &res2);
        LCHECK(!second.active() && !res2);                      // refused, and told so
        LCHECK(r.mgr.state() == State::Failed);
        LCHECK(r.log.count("platform.bring_up") == 1);          // never attempted again
        // the base-only and snapshot doors are the same door
        Result res3; auto base = r.mgr.acquire_base(ConsumerType::Ai, &res3);
        LCHECK(!base.active() && !res3);
        LCHECK(r.log.count("platform.bring_up") == 1);
        LCHECK(r.mgr.stats().failed_count == 1);                // one failure, not three
    }

    // 3. demand goes to zero and comes back - a reconnect is not a recovery
    {
        Rig r;
        auto keep = r.mgr.acquire(ConsumerType::Rtsp);
        r.platform.fail_at = FakePlatform::FailAt::BringUp; r.platform.fail_times = 1;
        std::string err;
        r.mgr.update_stream(r.stream, true, err);
        LCHECK(r.mgr.state() == State::Failed);
        const int after_fail = r.log.count("platform.bring_up");
        keep.release();                                          // demand -> 0
        LCHECK(r.mgr.stats().total_demand == 0);
        Result res; auto again = r.mgr.acquire(ConsumerType::Rtsp, &res);   // demand -> 1
        LCHECK(!again.active() && !res && r.mgr.state() == State::Failed);
        LCHECK(r.log.count("platform.bring_up") == after_fail);
    }
}
