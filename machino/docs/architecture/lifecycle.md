...
* `rtsp.max_clients` (default 4) caps the concurrent client *connections* —
  not the PLAYing sessions, because it is the connection that costs a thread.
  Connection number *n+1* is answered with `453 Not Enough Bandwidth` and
  closed: refused, not queued, so a client learns immediately instead of
  waiting on a stream that will not come.
