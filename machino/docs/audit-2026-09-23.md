...
# Nachtrag: Integritätsaudit des Baums

Anlass: im Laufe des 2026-09-23 haben zwei meiner eigenen Bearbeitungen
Dokumente still zerstört (`ap35-usb.md`, `pending-physical.md`, beide committet
und gepusht, beide repariert — Ursache und Hergang stehen in `ap35-usb.md`).
Die naheliegende Frage ist dann nicht „ist der Release in Ordnung", sondern
**„hat dieselbe Fehlerklasse noch woanders zugeschlagen"**.
...
**Seit dem CI-verifizierten RC1 ist keine einzige Quelldatei geändert.** Der
Auslieferungsstand ist unberührt; alles, was in diesem Zeitraum passiert ist,
war Dokumentation plus ein neues, rein lesendes Werkzeug. Damit kann die
Korruption den ausgelieferten Code nicht erreicht haben — nicht aus Vertrauen,
sondern weil der Diff leer ist.
...
| Quellstand seit RC1 | **unverändert** (siehe oben) |
| `sh -n` über alle Shellskripte im Baum | **alle ok**, inkl. Installer und `machino-manager` |
| Marker-Reste (`---TO---` / `---OLD---` / `===CUT===`) | **keine**, baumweit |
| NUL-Bytes in Text-, Quell- und YAML-Dateien | **keine** |
| Markdown ohne H1 | nur `README.md`, das mit `<div align="center">` beginnt — so gewollt |
| Wiederherstellung `pending-physical.md` | **22 Einfügungen, 0 Löschungen** gegen die letzte intakte Fassung — nichts verloren |
| Wiederherstellung `ap35-usb.md` | 13 gelöschte Zeilen, und zwar **exakt** die fünf beabsichtigten Korrekturen, nichts sonst |
| Löschungen seit RC1 insgesamt | 6 Zeilen, alle einzeln zugeordnet: 3 aus der RC1-Eintragung in `RELEASE.md`, 3 aus den Sicherheitskorrekturen in `evidence.md` und `pending-physical.md` |

## Das neue Werkzeug, bevor es auf Hardware läuft

`machino/tools/usb-inventory.sh` wurde gezielt darauf geprüft, ob es etwas
verändert:

```
Dateiumleitungen          keine   (die >-Treffer sind Bit-Shifts in $(( )))
modprobe/insmod/rmmod     keine
devmem                    keine   (liest ausschliesslich den regdump des Treibers)
rm/mv/chmod/chown/tee/dd  keine
```

Es liest und gibt aus, sonst nichts. Das ist die Voraussetzung dafür, es
jemandem in die Hand zu geben, der damit am offenen Gerät misst.

## Gerätezustand am Ende des Tages

```
uptime        9,6 h, kein Neustart
machino       pid 992, RSS 5136 kB, unverändert gegenüber der Messung 35 min zuvor
MemAvailable  21 348 kB
Listener      22, 554, 80, 127.0.0.1:85
geladene Module  8 — unverändert, es wurde nichts geladen
HPRT0         0x00001000 — unverändert
Front Door    HTTP 401 in 1,7 ms
```

Der RSS-Wert stand über 35 Minuten Leerlauf exakt still. Das entscheidet **P1
nicht** — dort geht es um die Böden über Kaltzyklen hinweg, nicht um Drift
innerhalb eines Leerlaufs — ist aber ein Datenpunkt in die freundliche
Richtung.

## Ergebnis

Die Korruption war **auf zwei Dokumente begrenzt** und ist behoben. Der
Quellstand, die Skripte und das Artefakt sind nachweislich unberührt. Was
weiterhin unbelegt bleibt, ist unverändert das, was oben unter Punkt 2 steht:
Hosttests und CI-Status lassen sich von dieser Maschine aus nicht prüfen.
