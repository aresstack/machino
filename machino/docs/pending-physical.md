...
| M5-6 | **DNS-Eigentum und die Rückgabe.** Mobilfunk aktiv, `resolv.conf` trägt die Resolver des Anbieters; zurück auf Ethernet, und die vorherigen müssen wieder dastehen. Die Momentaufnahme liegt seit dem Review vor der Hardwaresitzung unter `/var/run/machino-dns-baseline` und **überlebt einen Neustart des Daemons**; sie stirbt beim Reboot, was die richtige Lebensdauer ist. Hostseitig festgenagelt — am Gerät nachzuweisen bleibt, dass `/var/run` dort wirklich ein tmpfs ist und die Datei den Reboot also nicht übersteht |
...
| M6-2 | **`usb.mode=cellular` laedt die Module in der beabsichtigten Reihenfolge.** Insbesondere: bindet `cdc_ether` das ECM-Interface, BEVOR `option` (ggf. ueber `new_id`) an die Reihe kommt? Die Reihenfolge ist aus der Reuse-Map hergeleitet, nicht gemessen. Geht sie schief, entsteht ein Modem mit AT-Port und ohne Datenpfad. **Siehe M6-2a: die Reihenfolge allein reicht wahrscheinlich nicht** |
| M6-2a | **Die `new_id`-Loesung ist ein Bring-up-Behelf, kein Produktstand.** Der Gedanke "cdc_ether bindet MI_00 zuerst, also kann `option` es nicht mehr nehmen" setzt voraus, dass das Interface schon ECM ist. Ein fabrikneues EC200A steht laut Referenz aber auf **RNDIS** (`usbnet=3`) -- dann gibt es gar kein CDC-Ethernet-Interface, das cdc_ether binden koennte, und ein `new_id` verschluckt alles. Erst `AT+QCFG="usbnet",1` und eine Re-Enumeration machen daraus ECM, und der dynamische `option`-Match bleibt ueber die Re-Enumeration bestehen. **Nachzuweisen ist deshalb der Config-Descriptor des Geraets in BEIDEN Betriebsarten** (`lsusb -v`, Interfacenummern und Klassen). Danach -- und erst danach -- gehoert ein richtiger `option.ko`-Eintrag fuer `2c7c:6005` mit der passenden `.reserved`-Maske gebaut. Die Maske jetzt zu raten waere schlimmer als der Behelf: sie wuerde entweder den AT-Port oder das Netzwerkinterface dauerhaft verschlucken, und zwar auf eine Weise, die wie ein Hardwaredefekt aussieht |
