...
void PeerSession::on_readable() {
    ...
    for (;;) {
        ...
        if (is_stun(buf, (size_t)n)) {
            ...
            if (!have_peer_) {
                LOGI(MOD, "peer %u.%u.%u.%u:%u (stun ok)",
                     (peer_ip_ >> 24) & 255, (peer_ip_ >> 16) & 255,
                     (peer_ip_ >> 8) & 255, peer_ip_ & 255, peer_port_);
                dtls_.set_peer(peer_ip_, peer_port_);   // bind the DTLS cookie to this peer
            }
            have_peer_ = true;
            ...
            if (!dtls_.step()) {
                ...
                RuntimeStats::get().inc(&RuntimeCounters::webrtc_dtls_failures);
                return;
            }
            flush_dtls();
            if (dtls_.handshake_done() && !dtls_logged_) {
                ...
                if (dtls_.export_srtp(ours, theirs)) {
                    ...
                    RuntimeStats::get().inc(&RuntimeCounters::webrtc_srtp_failures);
